...
from .service import create_user_film, update_user_film_by_id
...
@router.post("/{user_id}/{film_id}", status_code=201,response_model=UserFilmResponse)
...
@router.patch("/films/{film_id}", response_model=UserFilmResponse)
async def update_user_film(
    film_id: UUID,
    request: UserAddFilm, session: AsyncSession = Depends(db_session), 
    sub = Depends(get_current_user)
):
    user = await get_user_by_id(sub["user_id"], session)
    if user is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="User does not exist",
    ...
    try:
        updated_user_film = await update_user_film_by_id(
            sub["user_id"], film_id, request.status, request.progress, session
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
